...
//     removeMouseListener(this);
  ...
  private void loadImage()
  {
    ...
    if( rr_pawn_image == null ){
      ...
      rr_pawn_image = tk.getImage( imageString(p, rs, rs) );
      tracker.addImage( rr_pawn_image, 0 );

      rb_pawn_image = tk.getImage( imageString(p, rs, bs) );
      tracker.addImage( rb_pawn_image, 0 );

      br_pawn_image = tk.getImage( imageString(p, bs, rs) );
      tracker.addImage( br_pawn_image, 0 );

      bb_pawn_image = tk.getImage( imageString(p, bs, bs) );
      tracker.addImage( bb_pawn_image, 0 );

      rr_rook_image = tk.getImage( imageString(r, rs, rs) );
      tracker.addImage( rr_rook_image, 0 );

      rb_rook_image = tk.getImage( imageString(r, rs, bs) );
      tracker.addImage( rb_rook_image, 0 );

      br_rook_image = tk.getImage( imageString(r, bs, rs) );
      tracker.addImage( br_rook_image, 0 );

      bb_rook_image = tk.getImage( imageString(r, bs, bs) );
      tracker.addImage( bb_rook_image, 0 );

      rr_knight_image = tk.getImage( imageString(n, rs, rs) );
      tracker.addImage( rr_knight_image, 0 );

      rb_knight_image = tk.getImage( imageString(n, rs, bs) );
      tracker.addImage( rb_knight_image, 0 );

      br_knight_image = tk.getImage( imageString(n, bs, rs) );
      tracker.addImage( br_knight_image, 0 );

      bb_knight_image = tk.getImage( imageString(n, bs, bs) );
      tracker.addImage( bb_knight_image, 0 );

      rr_bishop_image = tk.getImage( imageString(b, rs, rs) );
      tracker.addImage( rr_bishop_image, 0 );

      rb_bishop_image = tk.getImage( imageString(b, rs, bs) );
      tracker.addImage( rb_bishop_image, 0 );

      br_bishop_image = tk.getImage( imageString(b, bs, rs) );
      tracker.addImage( br_bishop_image, 0 );

      bb_bishop_image = tk.getImage( imageString(b, bs, bs) );
      tracker.addImage( bb_bishop_image, 0 );

      rr_queen_image = tk.getImage( imageString(q, rs, rs) );
      tracker.addImage( rr_queen_image, 0 );

      rb_queen_image = tk.getImage( imageString(q, rs, bs) );
      tracker.addImage( rb_queen_image, 0 );

      br_queen_image = tk.getImage( imageString(q, bs, rs) );
      tracker.addImage( br_queen_image, 0 );

      bb_queen_image = tk.getImage( imageString(q, bs, bs) );
      tracker.addImage( bb_queen_image, 0 );

      rr_king_image = tk.getImage( imageString(k, rs, rs) );
      tracker.addImage( rr_king_image, 0 );

      rb_king_image = tk.getImage( imageString(k, rs, bs) );
      tracker.addImage( rb_king_image, 0 );

      br_king_image = tk.getImage( imageString(k, bs, rs) );
      tracker.addImage( br_king_image, 0 );

      bb_king_image = tk.getImage( imageString(k, bs, bs) );
      tracker.addImage( bb_king_image, 0 );

      r_empty_image = tk.getImage( imageString(x, x, rs) );
      tracker.addImage( r_empty_image, 0 );

      b_empty_image = tk.getImage( imageString(x, x, bs) );
...
System.out.println("*** Flipped");
    ...
    ++flipCount;
    if (flipCount == 2) {
        System.startInstructionTrace();
    }
    if (flipCount == 4) {
        System.stopInstructionTrace();
    }
}
  }
