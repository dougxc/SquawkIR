...
//prtn("expectedSize ="+expectedSize);
    ...
    * Public constructor
    */
    public static Method create(VirtualMachine vm, InputContext ctx, Type parent, String name, String descriptor, int flags) throws VerificationException {
        int count         = countParameters(descriptor);
        if (count < 0) {
            throw ctx.verificationException();
        }
...
//prtn("ch="+(char)ch+"indx="+index);
            switch (ch) {
                ...
                case 'L': {
                    ...
                    res = parent.createType(desc.substring(start, index));
                    break;
                }
                case '[': {
                    int start = index;
